...
function GetCameraSettings(destinationcartesian,viewfromx,viewfromy,viewfromz) {
	let localpv=new CoordinateLocal(viewfromx,viewfromy,viewfromz);

	let cartesianpv=CoordinateLocal.ToCartesian(localpv,destinationcartesian);//本地转笛卡尔

	let heading=0;
	let pitch=0;
	let squrtxy=Math.sqrt(viewfromy*viewfromy+viewfromx*viewfromx);
	let squrtxyz=Math.sqrt(squrtxy*squrtxy+viewfromz*viewfromz);
	if(viewfromx>=0) heading=-Cesium.Math.PI_OVER_TWO-Math.asin(viewfromy/squrtxy);
	else heading=Math.asin(viewfromy/squrtxy)+Cesium.Math.PI_OVER_TWO;
	pitch=Math.asin(viewfromz/squrtxyz);
	return {
		heading:heading,
		pitch:-pitch,
		roll:0,
		poscartesian:cartesianpv
	};
}
...
function GetPointOnSameCircle(wgs84poi, heading, headingnew, pitch) {
	...
	let mercatorpoi = CoordinateMercator.fromWGS84(wgs84poi);
	...
	return CoordinateWGS84.fromMercatorxyh(x1, y1, wgs84poi.height);
}
