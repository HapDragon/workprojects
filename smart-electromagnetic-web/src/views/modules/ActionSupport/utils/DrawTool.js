...
export default class DrawTool {
	...
	drawLine(callback) {
		...
		this.mouseMove = (movement) => {
			...
			if (!cartesian) return;
			if (positions.length == 0) {
				this.CesiumTooltip.showAt(cartesian, "点击添加第一个点，右键取消");
			} else if (positions.length >= 2) {
				this.CesiumTooltip.showAt(cartesian, "双击完成，右键取消");
		...
		this.doubleClick = (movement) => {
			...
			if (typeof callback == "function") {
				let result = [];
				positions.some((item) => {
					result.push(cartesianToLonlat(item));
				});
				callback(result);
			}
			// 事件销毁
		...
		// 右键取消事件
		this.rightClick = (movement) => {
			// 事件销毁
			this.clear();
		};
		Bus.VM.$on("Scene_Mouse_Left_Click", this.leftClick, 10);
		Bus.VM.$on("Scene_Mouse_Move", this.mouseMove, 10);
		Bus.VM.$on("Scene_Mouse_DoubleLeft_Click", this.doubleClick, 10);
		Bus.VM.$on("Scene_Mouse_Right_Click", this.rightClick, 10);
	}
	/**
	 * @Author: dongnan
	 * @Description: 画面
	...
	stopDraw() {
		// 去除事件
		if (this.leftClick) {
			Bus.VM.$off("Scene_Mouse_Left_Click", this.leftClick);
			this.leftClick = null;
		}
		if (this.mouseMove) {
			Bus.VM.$off("Scene_Mouse_Move", this.mouseMove);
			this.mouseMove = null;
		}
		if (this.doubleClick) {
			Bus.VM.$off("Scene_Mouse_DoubleLeft_Click", this.doubleClick);
			this.doubleClick = null;
		}
		if (this.rightClick) {
			Bus.VM.$off("Scene_Mouse_Right_Click", this.rightClick);
...
class TooltipLabel {
	constructor(option) {
		this.viewer = option.viewer;
		this.color = Cesium.defaultValue(option.color, new Cesium.Color(0, 0, 0, 1));
		this.fontColor = Cesium.defaultValue(option.fontColor, Cesium.Color.WHITE);
...
class PolyLineEntity {
	constructor(option) {
		...
		this.lineColor = Cesium.defaultValue(option.color, Cesium.Color.BLACK);
		this.lineWidth = Cesium.defaultValue(option.width, 3);
		//实线实体
