...
<template>
    <div>
        <popdialog @close="onclickCancel" :title="title" :isshowdialog="showdialog" :width="'40vh'" :height="thisheight" :top="top" :left="left">
            <template v-slot:container>
                <el-form id="transmiterplace" label-width="90px">
                    ...
                    <el-form-item label="位置">
                        <span style="width:calc(100% - 20px);"><span>{{querymodelcopy.x}}</span>&nbsp;&nbsp;&nbsp;<span>{{querymodelcopy.y}}</span></span>
                        <el-icon :size="20" title="设置位置" @click="onclickloc()">
                            <Position />
                        </el-icon>
                    </el-form-item>
                    <el-form-item label="通讯灵敏度">
                        <el-input v-model="querymodelcopy.threshold"></el-input>
                    </el-form-item>
                    <!--<el-form-item label="计算模型">-->
                        <!--<el-select v-model="querymodelcopy.model">-->
                            ...
                            <!--&lt;!&ndash;<el-option label="ITU-R P.838" :value="4"></el-option>&ndash;&gt;-->
...
<script setup>
	...
	import {queryAnddrawSituation, removeSituation,removefieldpopup,querymodel} from "./situationdraw";
	import {entitypre} from "../situationdraw";
	...
	function onclickConfirm() {
		...
		if(Number.isNaN(querymodelcopy.threshold)===true){
			msgUtil.messagePrompt('info', '通讯灵敏度请设置为数字');
			return;
        }
		...
		querymodel.threshold=querymodelcopy.threshold;
	...
	onMounted(()=>{
		...
		querymodelcopy.threshold=querymodel.threshold;
