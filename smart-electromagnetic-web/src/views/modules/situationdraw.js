...
export const entitypre = "transimiterplaceentitypre";
...
function drawSituation(resdata,zoomto=true) {
	...
	function ongetterrainheight(updatedPositions){
		...
		const propertyquery=getquerymodel();
		propertyquery.unmannedVehicleX = propertyquery.x;
		propertyquery.unmannedVehicleY = propertyquery.y;
		...
		onleftclickhander = function (movement) {
			...
			propertyquery.x = pos.longitude;
			propertyquery.y = pos.latitude;

			const loadingInstance = ElLoading.service({ fullscreen: true,lock: true,
				text: '正在查询，请稍侯...',
				background: "rgba(0, 0, 0, 0.7)", })
			post(
				emitradioapi.queryradiowave,
				propertyquery,
			)
				.then((radiores) => {
					loadingInstance.close();
					if (radiores.code == '0'&&radiores.data!=null&&radiores.data.path!=null) {
						SetEntity({
						...
						//添加弹框
						resourcePopEvents.push(addResourceWellPop({
							contenthtml: `
<div style="width:80%;height:calc(100% - 1vh);margin:auto;margin-bottom:1vh;display: flex;flex-direction: column;
    justify-content: space-evenly;">
				...
				<div class="rowitem"><span class="col">场强:</span><span class="col">${radiores.data.path.fieldStrength?radiores.data.path.fieldStrength+' dbm ':'空值'}</span></div>
				<div class="rowitem"><span class="col">时延:</span><span class="col">${radiores.data.path.delay?radiores.data.path.delay+' ns':'空值'}</span></div>
</div>				
`,
							style: 'width:24.5vh;height:24vh;',
