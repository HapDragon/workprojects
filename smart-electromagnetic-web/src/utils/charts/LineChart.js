...
import { fontSize_VW, fontSize_VH } from './chartSize';
/**
 * 初始化图表，需要传入图表容器的id
...
function setOption(myChart, data, yAxisName, color){
    ...
    for(let i = 0; i < data.length; i++){
        ...
        xAxisArr.push({ value: xobj, textStyle: {color: '#FFFFFFB3'} });
    }
...
function dynamicSetOption(myChart, data, yAxisName, color, count, interval){
    ...
    for(let m = 0; m < count; m++){
        ...
        xAxisArr.push({ value: xobj, textStyle: {color: '#FFFFFF40'} });
    }
    numcount = count;
...
function _initData(data){
 ...
 function _initOption(xAxisArr, yAxisName, color, dataArr){
    ...
    let option = {
        ...
        legend: {
            itemWidth: fontSize_VH(2),
			itemHeight: fontSize_VH(0.8),
            top: fontSize_VH(2),
            textStyle:{
                color: '#FFFFFF',
                fontSize: fontSize_VH(1.2)
    ...
    for(let i = 0; i < dataArr.length; i++){
        let serie = {
            ...
            symbolSize: fontSize_VH(0.5), // 设置节点的大小
            symbol: 'circle',   //设置了这一项，itemStyle才会起效（相当于是自定义折线拐点展示的图形）
            itemStyle: {
                color: 'white',
                borderColor: color[i],
                borderWidth: fontSize_VH(0.15),
                borderType: 'solid',
            },
            lineStyle: {
                color: color[i]
