...
export async function drawDoubleImageCanvas(img1, img2, textOptions) {
	...
	if (Cesium.defined(textOptions)) {
		text1 = Cesium.defined(textOptions.text) ? textOptions.text : text1;
		text2 = Cesium.defined(textOptions.text2) ? parseFloat(textOptions.text2) : text2;
		minNum = Cesium.defined(textOptions.minNum) ? parseFloat(textOptions.minNum) : minNum;
		maxNum = Cesium.defined(textOptions.maxNum) ? parseFloat(textOptions.maxNum) : maxNum;
		// 第一个文字样式
		if (Cesium.defined(textOptions.fontOptions)) {
			font1 = Cesium.defined(textOptions.fontOptions.font) ? textOptions.fontOptions.font : font1;
			fillStyle1 = Cesium.defined(textOptions.fontOptions.fillColor) ? textOptions.fontOptions.fillColor : fillStyle1;
			strokeStyle1 = Cesium.defined(textOptions.fontOptions.strokeColor) ? textOptions.fontOptions.strokeColor : strokeStyle1;
		}
		// 第二个文字样式
		if (Cesium.defined(textOptions.fontOptions2)) {
			font2 = Cesium.defined(textOptions.fontOptions2.font) ? textOptions.fontOptions2.font : font2;
			fillStyle2 = Cesium.defined(textOptions.fontOptions2.fillColor) ? textOptions.fontOptions2.fillColor : fillStyle2;
			strokeStyle2 = Cesium.defined(textOptions.fontOptions2.strokeColor) ? textOptions.fontOptions2.strokeColor : strokeStyle2;
		}
	}
	// 确定canvas大小
	...
	// 绘制数字
	drawTextPlacing(text2.toString(), 8, circleDiameter, circleDiameter / 2 - 8, {
		font: font2,
	...
	function drawLongText(text, img, fontOptions) {
		...
		let fontSize = context.measureText(text.trim()).width / text.trim().length;
		if (context.measureText(text.trim()).width + img.width + 20 <= circleDiameter) {
			context.strokeText(text.trim(), circleDiameter / 2 + img.width / 2, circleDiameter / 2 + 8);
			context.fillText(text.trim(), circleDiameter / 2 + img.width / 2, circleDiameter / 2 + 8);
			// 绘制图片
			let leftOffset = circleDiameter / 2 + img.width / 2 + 5 - img.width - (text.trim().length * fontSize) / 2 - 15;
			let topOffset = circleDiameter / 2 + 8 - img.height / 2 + fontSize / 2;
			context.drawImage(img, leftOffset, topOffset);
		} else {
			context.strokeText(text.trim(), circleDiameter / 2, circleDiameter / 2 + 8);
			context.fillText(text.trim(), circleDiameter / 2, circleDiameter / 2 + 8);
		}
	}
	/**
	 * @Author: dongnan
	...
	function drawTextPlacing(text, space, totalLength, startY, fontOptions) {
		...
		let textLength = context.measureText(text).width + (list.length - 1) * space;
